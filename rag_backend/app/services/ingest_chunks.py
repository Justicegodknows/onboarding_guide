...
def ingest_chunks(source: str = "google_drive", allow_local_fallback: bool = True):
    rag = RAGService()

    ingest_source = (source or "").strip().lower()
    if ingest_source not in {"google_drive", "local"}:
        raise ValueError("source must be either 'google_drive' or 'local'.")
