          ...
          ${isDragging
            ...
            {queue.length > 0 && (
                <div className="flex flex-col gap-2">
                    <h3 className="text-sm font-bold text-gray-600 uppercase tracking-wide">Upload Queue</h3>
                    {queue.map((entry, i) => (
                        <div
                            key={i}
                        ...
                        >
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <span className="truncate font-medium text-gray-800">{entry.file.name}</span>
                                <span className="text-xs text-gray-400 shrink-0">{(entry.file.size / 1024).toFixed(1)} KB</span>
                            </div>
                    ...
                    <button
                        onClick={() => setQueue([])}
                        className="text-xs text-gray-400 hover:text-gray-700 self-end mt-1 underline"
                    >
                        Clear queue
                    </button>
                </div>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {documents.map((doc) => (
                                    <tr key={doc.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-gray-400">{doc.uploaded_at ?? "—"}</td>
                                        {isAdmin && (
...
function FileIcon({ ext }: { ext: string }) {
    ...
    return (
        <span className={`text-xs font-bold uppercase ${colors[ext.toLowerCase()] ?? "text-gray-400"}`}>
            {ext}
        </span>
    );
}
...
function StatusBadge({ entry }: { entry: UploadEntry }) {
    if (entry.status === "pending") return <span className="text-gray-400 text-xs">Waiting…</span>;
